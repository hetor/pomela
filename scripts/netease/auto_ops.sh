...
function doRefund() {
	...
	mysql -uhaitao_read -pOfX5LtGcI3Yad1 -h10.120.154.154 -P6000 haitao-mirror <<-EOF 1>result.t 2>err.t
        select state,trade_id,pay_method,hqg_extra_fee,real_pay_method_order_id,from_unixtime(deliver_time) as deliver_time,from_unixtime(pay_time) as pay_time,from_unixtime(succ_time) as succ_time,from_unixtime(close_time) as close_time from orders where out_id = '$ORDER_OUT_ID';
	
            select out_id,order_out_id,order_item_out_id,from_unixtime(create_time) as create_time,from_unixtime(transfer_time) as transfer_time,amount,state,pay_method,account_out,account_in,type,from_type
	    from trade_refund_transfer where order_out_id='$ORDER_OUT_ID';
	
        select gorder_id,from_unixtime(create_time) as create_time,from_unixtime(update_time) as update_time,from_unixtime(close_time) as close_time,from_unixtime(pay_time) as pay_time,state,real_pay_method,pay_method,wyb_order_id,real_pay_method_order_id,pay_method_sub_type,owner_type,real_pay_method_account,is_merge_payment from trade where order_id = '$ORDER_OUT_ID';
