...
function doRefund() {
	...
	mysql -uhaitao_read -pOfX5LtGcI3Yad1 -h10.120.154.154 -P6000 haitao-mirror <<-EOF 1>result.t 2>err.t
	    ...
	    select id,order_id,gorder_id,user_name,user_id,create_time,update_time,close_time,pay_time,pre_expire_time,origin_total_amount,\
            ...
            real_pay_method_order_id,pay_method_sub_type,db_update_time,owner_type,real_pay_method_account,is_merge_payment\ 
        from trade where order_id = '$ORDER_OUT_ID';
	EOF

    cat result.t | awk '\
        NR==2{printf "%10s %10s %10s %10s\n",$1,$2,$3,$4}\
        NR==4{printf "%10s %10s %10s %10s\n",$1,$2,$3,$4}\
